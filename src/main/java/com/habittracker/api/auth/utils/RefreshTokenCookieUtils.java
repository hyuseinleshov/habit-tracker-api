...
public class RefreshTokenCookieUtils {
  ...
  public RefreshTokenCookieUtils(
      @Value("${refresh-token.expiration-duration}") Duration refreshTokenDuration) {
    this.refreshTokenDuration = refreshTokenDuration;
  }

  public void addRefreshTokenCookie(String refreshToken, HttpServletResponse response) {
    response.setHeader(
        COOKIE_HEADER, buildRefreshTokenCookie(refreshToken, refreshTokenDuration).toString());
  }
